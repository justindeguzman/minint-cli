...
function main () {
  ...
  const babel = path.resolve(__dirname, '../node_modules/babel-cli/bin/babel.js').replace(/ /g, '\\ ')
  const flags = '--ignore /node_modules/,/test/ --out-dir '
  execSync(`${babel} ${functionDir.replace(/ /g, '\\ ')} ${flags} ${tmp.replace(/ /g, '\\ ')}`)
